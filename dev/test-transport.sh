...
for pattern in test_transport.py test_barrier.py test_exchange.py test_transfer.py test_snapshot.py; do
  PYTHONPATH="$PWD/python" "$PYJAVA_TEST_PYTHON" -m unittest discover \
    -s python/pyjava/tests -p "$pattern" -v
done
# Load by package name so Ray workers can unpickle the callbacks.
# discover would name the module test_ray_snapshot, which workers cannot import.
PYTHONPATH="$PWD/python" "$PYJAVA_TEST_PYTHON" -m unittest pyjava.tests.test_ray_snapshot -v
...
suites="tech.mlsql.arrow.python.PartitionLifecycleSpec,tech.mlsql.arrow.python.IdleWorkerPoolSpec,tech.mlsql.arrow.python.PythonWorkerFactorySpec,tech.mlsql.arrow.python.ArrowTransportSpec,tech.mlsql.arrow.python.SparkSocketTransportSpec,tech.mlsql.arrow.python.SparkSocketServeSpec,tech.mlsql.arrow.python.SparkSharedSnapshotSpec,tech.mlsql.test.ArrowUtilsSpec,tech.mlsql.test.ArrowConvertersSpec,tech.mlsql.test.WowRowEncoderSpec,tech.mlsql.test.SparkUtilsSpec"
